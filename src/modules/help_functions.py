...
def help_command(option_config, params):
    if len(params)<2:
        ...
        if params[1]=="assets":
            ...
            print("It runs some tests.")
            print("We have the following types of tests:")
            print("(1). `Component` tests and checks on the presence of a component (compiler, emulator, roms, tools, interpreter, library, etc.)")
            print("(2). `Unit-tests` on python scripts")
            print("(3). `Self` tests on `xl` itself")
            print("(4). `Output` tests for the output of a compiled program (for the `stdio` target)")
            print("(5). `Compilation` tests the number of of binaries produced by several native and cross-compilers")
            print("")
            print("<params>")
            print("If nothing or `all` or `everything` is passed to <params>, then all tests are performed, i.e., tests of type (1), (2), (3), (4), (5).")
            print("If `standard` is passed to <params>, then it performs several standard tests, i.e., tests of type (1), (2), (3), (4), excluding (5).")
            print("")
            print("(1) If `components` is to passed to <params>, then tests on the presence of components are performed, i.e., tests of type (1).")
            print("(2) If 'unit-tests' is passed to <params>, then it runs unit-tests of several Python functions of the scripts, i.e., tests of type (2).")
            print("(3) If 'self' is passed to <params>, then it tests itself by performing a sequence of 'xl' commands, i.e., tests of type (3).")
            print("(4) If 'output' is passed to <params>, then it tests the output to `stdio` of several examples, i.e., tests of type (4).")
            print("(5) If 'compilation' is passed to <params>, then it tests the number of produced binaries by the found compilers, i.e., tests of type (5).")
            ...
            print("")
            print("If 'cc65', 'z88dk, 'cmoc', 'ack', 'cc6303', 'vbcc', 'lcc1802', 'tms9900-gcc' or 'native' is passed to <params>, it compiles a test program using the corresponding compiler.")
            print("If 'z88dk_alt' is passed to <params>, then it compiles a different test program the Z88DK compiler.")
