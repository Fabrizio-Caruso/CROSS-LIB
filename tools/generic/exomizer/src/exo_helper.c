                  ...
                  struct crunch_info *infop) /* OUT */
{
    ...
    {
        LOG(LOG_BRIEF, (" Crunched data enlarged %d bytes",
                        outlen - inlen));
        if (inlen != 0)
        {
            LOG(LOG_BRIEF, (" (%0.2f%%)", 100.0 * (outlen - inlen) / inlen));
        }
        LOG(LOG_BRIEF, (".\n"));
    }
...
void print_license(void)
{
    LOG(LOG_WARNING,
        ("----------------------------------------------------------------------------\n"
         "Exomizer v3.1.3b0 Copyright (c) 2002-2023 Magnus Lind. (magli143@gmail.com)\n"
         "----------------------------------------------------------------------------\n"));
    LOG(LOG_WARNING,
        ("This software is provided 'as-is', without any express or implied warranty.\n"
         "In no event will the authors be held liable for any damages arising from\n"
