...
static uint8_t getTileAt(uint8_t roomType, Position pos) {
    ...
    // Pattern 20: Top-Left Corner Block (Redesigned)
    // Ensures 4x4 top-left is empty. Places a block in the center-top area instead.
    if (roomType == 20) {
        // Keep top-left 4x4 empty (x<=3, y<=3)
        if (x <= 3 && y <= 3) return 0;
        
        // Place a wall block in the upper-middle area
        if ((x >= cx - 2 && x <= cx + 2) && (y >= 4 && y <= 6)) {
            return 1;
        }
        return 0;
    }

    // Pattern 21: Bottom-Right Corner Block (Redesigned)
    // Ensures 4x4 bottom-right is empty. Places a block in the center-bottom area instead.
    if (roomType == 21) {
        // Keep bottom-right 4x4 empty (x>=W-4, y>=H-4)
        if (x >= ROOM_WIDTH - 4 && y >= ROOM_HEIGHT - 4) return 0;
        
        // Place a wall block in the lower-middle area
        if ((x >= cx - 2 && x <= cx + 2) && (y >= cy + 1 && y <= cy + 3)) {
            return 1;
        }
        return 0;
    }
    ...
    // Pattern 29: Inner Box Border (Redesigned)
    // Ensures 4x4 corners are empty. Places a box in the center with gaps.
    if (roomType == 29) {
        // Keep top-left 4x4 and bottom-right 4x4 empty
        if ((x <= 3 && y <= 3) || (x >= ROOM_WIDTH - 4 && y >= ROOM_HEIGHT - 4)) return 0;
        
        uint8_t bx = cx - 3, by = cy - 3;
        uint8_t bw = 6, bh = 6;
        
        // Check if inside the box area
        if ((x >= bx && x <= bx + bw) || (y >= by && y <= by + bh)) {
            // Draw border with gaps every 2 tiles to ensure passage
            if (x == bx || x == bx + bw || y == by || y == by + bh) {
                uint8_t gapCheck = (x + y) % 2;
                if (gapCheck != 0) return 1;
            }
        }
        return 0;
    }

    // Pattern 30: Outer Box Border (Redesigned)
    // Ensures 4x4 corners are empty. Places a box near edges with gaps.
    if (roomType == 30) {
        // Keep top-left 4x4 and bottom-right 4x4 empty
        if ((x <= 3 && y <= 3) || (x >= ROOM_WIDTH - 4 && y >= ROOM_HEIGHT - 4)) return 0;
        ...
        // Check if inside the box area
        if ((x >= bx && x <= bx + bw) || (y >= by && y <= by + bh)) {
            // Draw border with gaps every 3 tiles to ensure passage
            if (x == bx || x == bx + bw || y == by || y == by + bh) {
                uint8_t gapCheck = (x + y) % 3;
                if (gapCheck != 0) return 1;
            }
        }
        return 0;
    ...
    // Pattern 35: Diagonal Corner Blocks (Redesigned)
    // Ensures 4x4 top-left and bottom-right are empty.
    // Places blocks in the other two corners (Top-Right and Bottom-Left).
    if (roomType == 35) {
        // Keep top-left 4x4 and bottom-right 4x4 empty
        if ((x <= 3 && y <= 3) || (x >= ROOM_WIDTH - 4 && y >= ROOM_HEIGHT - 4)) return 0;
        
        // Place block in Top-Right corner area
        if ((x >= ROOM_WIDTH - 5 && x <= ROOM_WIDTH - 2) && (y >= 1 && y <= 3)) {
            return 1;
        }
        
        // Place block in Bottom-Left corner area
        if ((x >= 1 && x <= 3) && (y >= ROOM_HEIGHT - 5 && y <= ROOM_HEIGHT - 2)) {
            return 1;
        }
...
static void drawInitialScreen(void) {
    ...
    uint16_t roomNum = (uint16_t)(g_game.currentRoom + 1);
