...
void update_game(void)
{
    ...
    if (_XL_FIRE(inp) && (fire_tick%4==0) && active_bullets < max_bullets_allowed){
        ...
        for (k=0;k<(fire_mode?3:1);k++){
            ...
            /* immediate hit test for the just fired bullet */
            hit_found = 0;
            ...
            if (hit_found) continue;          /* do not create / draw a bullet */
    ...
    for (i=0;i<MAX_BULLETS;i++){
        ...
        /* do not erase the player sprite */
        if (!(ox==player_x && (oy==player_y || oy==player_y+1)))
            _XL_DELETE(ox,oy);
    ...
    /* enemy movement, drawing and item handling unchanged */
    for (i=0;i<MAX_ENEMIES;i++){
        if (!enemy_active[i]) continue;
