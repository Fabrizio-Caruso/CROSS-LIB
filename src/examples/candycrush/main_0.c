...
static void apply_gravity(void)
{
    ...
    /* Show the cleared gaps before the fall begins */
    draw_all();
...
int main(void)
{
    ...
    for (;;) {
        ...
        while (!g_over) {
            ...
            if (!g_sel) {
                /* --- Cursor mode: arrows move, FIRE selects --- */
                ...
                /* --- Selected mode: arrows swap, FIRE deselects --- */
                if (_XL_FIRE(input)) {
                    g_sel = 0;
                    _XL_TICK_SOUND();
            ...
            /* Redraw only cells that may have changed visual state */
