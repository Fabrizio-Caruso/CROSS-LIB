...
static void mark_bomb(uint8_t x, uint8_t y)
{
    uint8_t dx;
    uint8_t dy;
    uint8_t nx;
    ...
    for (dy = 0; dy < 3; dy++) {
        for (dx = 0; dx < 3; dx++) {
            ...
            if (dy == 2 && y == BS - 1) continue;
            if (dx == 0 && x == 0) continue;
            if (dx == 2 && x == BS - 1) continue;
            nx = (uint8_t)(x + dx - 1);
            ny = (uint8_t)(y + dy - 1);
            g_mrk[nx][ny] = 1;
        }
...
static void try_swap(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
    ...
    bomb = 0;
    if (g_board[x1][y1] == BOMB) {
        mark_bomb(x1, y1);
        bomb = 1;
    }
    if (g_board[x2][y2] == BOMB) {
        mark_bomb(x2, y2);
        bomb = 1;
    }
