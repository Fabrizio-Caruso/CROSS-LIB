...
{        
    ...
    {
        _XL_DRAW((i&15)*CHAR_SKIP+COL_OFFSET,(i/16)*LINE_SKIP+ROW_OFFSET,tiles[i],_XL_WHITE);

    }
    #else
    for(i=0;i<_XL_NUMBER_OF_TILES;++i)
    {
        _XL_DRAW((i&7)*CHAR_SKIP+COL_OFFSET,(i/8)*LINE_SKIP+ROW_OFFSET,tiles[i],_XL_WHITE);
