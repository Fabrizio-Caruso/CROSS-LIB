...
// All patterns guarantee a path from (1,1) to (W-2, H-2).
static uint8_t getTileAt(uint8_t roomType, Position pos) {
    ...
    // Pattern 5: Vertical Corridors (Walls at x=3,7,11,15 - but 15 is border)
    if (roomType == 5) {
        if (x % 4 == 3 && y > 2 && y < ROOM_HEIGHT - 3) return 1;
        return 0;
    }

    // Pattern 6: Horizontal Corridors (Walls at y=3,7,11,15 - but 15 is border)
    if (roomType == 6) {
        if (y % 4 == 3 && x > 2 && x < ROOM_WIDTH - 3) return 1;
        return 0;
    }

    // Pattern 7: L-Shaped Walls in corners (Top-Left and Bottom-Right)
    if (roomType == 7) {
        // Top-Left L (small)
        if ((x <= 2 && y <= 2)) return 1;
        // Bottom-Right L (small)
        if ((x >= ROOM_WIDTH - 3 && y >= ROOM_HEIGHT - 3)) return 1;
        return 0;
    }

    // Pattern 8: Top and Bottom Walls (with gaps every 3 tiles)
    if (roomType == 8) {
        if (y <= 2 && x % 3 != 0 && x > 1 && x < ROOM_WIDTH - 2) return 1;
        if (y >= ROOM_HEIGHT - 3 && x % 3 != 0 && x > 1 && x < ROOM_WIDTH - 2) return 1;
        return 0;
    }
    ...
    // Pattern 10: Spiral-like walls (simplified inner box)
    if (roomType == 10) {
        uint8_t inX = 3, outX = ROOM_WIDTH - 4;
        uint8_t inY = 3, outY = ROOM_HEIGHT - 4;
        // Inner border of a box
        if ((x >= inX && x <= outX) || (y >= inY && y <= outY)) {
            if (x == inX || x == outX || y == inY || y == outY) return 1;
        }
        return 0;
    }
    ...
    // Pattern 14: Box in center (hollow)
    if (roomType == 14) {
        uint8_t bx = ROOM_WIDTH / 2 - 3;
        uint8_t by = ROOM_HEIGHT / 2 - 3;
        uint8_t bw = 6;
        uint8_t bh = 6;
        // Only draw border of the box, not fill
...
// Draw the current room and entities
static void drawGame(void) {
    _XL_CLEAR_SCREEN();
    
    uint8_t type = getRoomType(g_game.currentRoom);
    
    // 1. Draw static background (Floor/Walls/Exit)
    for (uint8_t y = 0; y < ROOM_HEIGHT; y++) {
        for (uint8_t x = 0; x < ROOM_WIDTH; x++) {
            ...
            if (isWall) {
                _XL_DRAW(x, y, TILE_WALL, COLOR_WALL);
            } else {
                // Check for exit
                if (isExit(g_game.currentRoom, pos)) {
                    _XL_DRAW(x, y, TILE_EXIT, COLOR_EXIT);
                } else {
                    _XL_DRAW(x, y, TILE_FLOOR, COLOR_FLOOR);
                }
            }
        }
    }
    
    // 2. Draw dynamic entities (Enemies and Player)
    for (uint8_t i = 0; i < g_game.numEnemies; i++) {
        if (g_game.enemies[i].alive) {
    ...
    // Note: Ensure UI coordinates are within screen limits if XSize/YSize are small.
    // Assuming standard Cross-Lib screen is large enough for 16x16 + UI.
    _XL_SET_TEXT_COLOR(COLOR_TEXT);
    
    // Place UI below the room grid
    ...
    // Display room number (1-based for user)
...
static void movePlayer(int8_t dx, int8_t dy) {
    ...
    if (isExit(g_game.currentRoom, newPos)) {
        ...
        // Initialize enemies for new room
