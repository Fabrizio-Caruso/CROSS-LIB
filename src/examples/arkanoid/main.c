...
/*
  ...
  Therefore total columns = XSize
  total rows     = YSize
*/
#define SCREEN_W XSize
#define SCREEN_H YSize
...
static uint8_t clamp_row(uint8_t y);
...
static uint8_t can_show_score(void);
...
static uint8_t center_x(uint8_t len)
{
    /* Total columns is XSize, max x is XSize - 1. */
    if ((uint16_t)len > (uint16_t)XSize) {
        return 0;
    }

    return (uint8_t)(((uint16_t)XSize - (uint16_t)len) / 2);
}
...
static uint8_t clamp_row(uint8_t y)
{
    ...
    if ((uint16_t)y >= (uint16_t)YSize) {
        return (uint8_t)((uint16_t)YSize - 1);
    }
...
{
    char buf[32];
    uint16_t total = (uint16_t)XSize;
    uint8_t len;
    uint8_t x;
    uint8_t i;

    if ((uint16_t)YSize == 0 || total == 0) {
        return;
    }
...
/* Paddle is on the last valid row: YSize - 1 */
static uint8_t paddle_row(void)
{
    if ((uint16_t)YSize > 0) {
        return (uint8_t)((uint16_t)YSize - 1);
...
static void init_blocks(void)
{
    ...
    /*
       Blocks must not occupy the paddle row, and we also leave one empty
       row above the paddle for the ball's starting position.

       Valid block rows:
         top_row + 1 .. paddle_row - 2
    */
    if ((uint16_t)paddle_row() >= (uint16_t)top_row() + 2) {
        max_rows = (uint16_t)((uint16_t)paddle_row() - (uint16_t)top_row() - 2);
    } else {
        max_rows = 0;
    }
    ...
    for (r = 0; r < rows; ++r) {
        ...
        /* Safety: keep at least one empty row above the paddle. */
        if ((uint16_t)y + 2 > (uint16_t)paddle_row()) {
            break;
        }
        ...
        /*
           With max x = XSize - 1, and margin m on both sides:
             valid x range is m .. XSize - 1 - m
             loop condition: x < XSize - m
        */
...
static uint8_t score_x(void)
{
    /* If there is room for label + space + 5 digits, use x = 7. */
    if ((uint16_t)XSize >= 12) {
        return 7;
    }

    /* Otherwise put the number near the left edge. */
    if ((uint16_t)XSize >= 6) {
        return 1;
    }
...
static uint8_t lives_label_x(void)
{
    ...
    return (uint8_t)((uint16_t)XSize - 7);
}
...
static void refresh_paddle_cells(uint8_t old_x)
{
    ...
    if (max_x >= (uint16_t)XSize) {
        max_x = (uint16_t)((uint16_t)XSize - 1);
    }
...
static void new_game(void)
{
    ...
    /* Center paddle in total width XSize. */
    if ((uint16_t)XSize >= (uint16_t)paddle_w) {
        paddle_x = (uint8_t)(((uint16_t)XSize - (uint16_t)paddle_w) / 2);
    } else {
        paddle_x = 0;
    }
...
static uint8_t next_x(uint8_t x, uint8_t dir)
{
    ...
    if (dir == V_RIGHT) {
        /* Max x is XSize - 1. */
        if ((uint16_t)XSize > 0 && (uint16_t)x + 1 < (uint16_t)XSize) {
            return (uint8_t)((uint16_t)x + 1);
        }

        if ((uint16_t)XSize > 0) {
            return (uint8_t)((uint16_t)XSize - 1);
        }
...
static uint8_t next_y(uint8_t y, uint8_t dir)
{
    ...
    if (dir == V_RIGHT) { /* down */
        /* Max y is YSize - 1. */
        if ((uint16_t)YSize > 0 && (uint16_t)y + 1 < (uint16_t)YSize) {
            return (uint8_t)((uint16_t)y + 1);
        }
...
static void normalize_velocity(void)
{
    ...
    if (ball_vx == V_ZERO) {
        if ((uint16_t)ball_x == 0) {
            ball_vx = V_RIGHT;
        } else if ((uint16_t)XSize > 0 &&
                   (uint16_t)ball_x + 1 >= (uint16_t)XSize) {
            ball_vx = V_LEFT;
        } else {
    ...
    if (ball_vy == V_ZERO) {
        ...
        } else if ((uint16_t)YSize > 0 &&
                   (uint16_t)ball_y + 1 >= (uint16_t)YSize) {
            ball_vy = V_LEFT;  /* up */
        } else {
...
static uint8_t move_ball(void)
{
    ...
    /* Reflect side walls. Max x is XSize - 1. */
...
{
    uint8_t w;
    uint16_t maxw = (uint16_t)XSize;

    if ((uint16_t)XSize >= 5) {
        w = 5;
    } else if ((uint16_t)XSize >= 3) {
        w = 3;
    } else {
        w = 1;
    }
...
uint8_t main(void)
{
    ...
    while (1) {
        ...
        while (state == STATE_PLAYING) {
            ...
            if (_XL_RIGHT(input)) {
                /*
                   Total columns is XSize, so valid paddle starts are:
                     0 .. XSize - paddle_w
                */
                if ((uint16_t)XSize >= (uint16_t)paddle_w) {
                    max_paddle_x = (uint16_t)((uint16_t)XSize - (uint16_t)paddle_w);
                } else {
                    max_paddle_x = 0;
                }
