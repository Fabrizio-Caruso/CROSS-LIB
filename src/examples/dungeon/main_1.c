...
/* Place enemies very far from player start */
static void place_enemies(uint8_t lvl)
{
    ...
    /* Minimum Manhattan distance: at least half the map diagonal */
...
/* ===== Level type: maze with 2-wide corridors ===== */
...
static void gen_open_room(uint8_t lvl)
{
    ...
    /* Border walls, interior all open */
    for (y = 0; y < YSize; y++) {
        for (x = 0; x < XSize; x++) {
    ...
    /* Place one or two roughly central square wall blocks */
    uint8_t num_blocks = (uint8_t)(1 + _XL_RAND() % 2); /* 1 or 2 */
    for (i = 0; i < num_blocks; i++) {
        /* Square size: 3 to 5 */
        uint8_t sz = (uint8_t)(3 + _XL_RAND() % 3);
        /* Position roughly central with some randomness */
        uint8_t bx = (uint8_t)((XSize / 4) + _XL_RAND() % (XSize / 2));
        uint8_t by = (uint8_t)((YSize / 4) + _XL_RAND() % (YSize / 2));

        /* Clamp so the square stays inside */
    ...
    /* Player at top-left, door at bottom-right */
    px = 1; py = 1;
...
static void gen_flat_16x16(uint8_t lvl)
{
    ...
    /* Carve the 16x16 open room (no internal walls) */
    ...
    /* Player at top-left of room, door at bottom-right */
    px = (uint8_t)(ox + 1);
    py = (uint8_t)(oy + 1);
...
static void gen_level(uint8_t lvl)
{
    ...
    if (type == 0) {
        /* ~1/4: large open room with central square walls */
        gen_open_room(lvl);
    } else if (type == 2) {
        /* ~1/4: 16x16 flat room, no internal walls */
        gen_flat_16x16(lvl);
    } else {
        /* ~1/2: maze with 2-wide corridors */
        gen_maze_level(lvl);
    }
}
...
static void move_enemies(void)
{
    ...
    for (i = 0; i < enemy_count; i++) {
        ...
        /* Enemies can only move into empty cells — doors block them */
...
static void update_bullets(void)
{
    ...
    for (i = 0; i < MAX_BULLETS; i++) {
        ...
        /* Bullets stop at walls and both door types */
...
static void try_move(uint8_t dir)
{
    ...
    /* Player cannot enter walls or closed doors */
    uint8_t cell = grid[nx][ny];
...
int main(void)
{
    ...
    while (1) {
        ...
        while (game_state == STATE_PLAYING) {
            ...
            /* Enemies move every ENEMY_MOVE_INTERVAL frames (slower) */
