...
{
    uint8_t col = invincible_timer ? _XL_YELLOW : _XL_CYAN;
    uint8_t t0,t1;
...
void update_game(void)
{
    ...
    for (i=0;i<MAX_BULLETS;i++){
        ...
        _XL_DELETE(ox,oy);
        if (bullet_x[i]<XSize && bullet_y[i]>0 && bullet_y[i]<YSize){_XL_DRAW(bullet_x[i],bullet_y[i],_TILE_0,_XL_YELLOW);}
    ...
    for (i=0;i<MAX_ENEMIES;i++){
        ...
        if (dx && enemy_x[i]+dx<XSize && enemy_x[i]+dx>0 && !enemy_occupies(enemy_x[i]+dx,enemy_y[i])) nx=enemy_x[i]+dx; else dx=0;
        if (dy && enemy_y[i]+dy<YSize-1 && enemy_y[i]+dy>=PLAY_TOP && !enemy_occupies(enemy_x[i],enemy_y[i]+dy)) ny=enemy_y[i]+dy; else dy=0;
        occupied = 0;
        for (j=0;j<MAX_ENEMIES;j++) if (j!=i && enemy_active[j]){
            if ((enemy_x[j]==nx && enemy_y[j]==ny) || (enemy_x[j]==nx && enemy_y[j]+1==ny)){occupied=1;break;}
        }
        if (!occupied){
            if (dx<0) enemy_dir[i]=0; else if (dx>0) enemy_dir[i]=1;
