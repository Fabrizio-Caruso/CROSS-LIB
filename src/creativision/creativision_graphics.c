...
#define VDP_Data_Write 0x3000
#define VDP_Status_Write 0x3001

#define _DRAW(x,y,image) vpoke(BASE+x+X_OFFSET+(y+Y_OFFSET)*XSize,image->_imageData);
#define _DELETE(x,y)     vpoke(BASE+x+X_OFFSET+(y+Y_OFFSET)*XSize,' ');
#define _DRAW_VERTICAL_WALL(x,y)  vpoke(BASE+x+X_OFFSET+(y+Y_OFFSET)*XSize, _VERTICAL_BRICK);
#define _DRAW_HORIZONTAL_WALL(x,y)  vpoke(BASE+x+X_OFFSET+(y+Y_OFFSET)*XSize, _HORIZONTAL_BRICK);
#define _DRAW_BROKEN_WALL(x,y)  vpoke(BASE+x+X_OFFSET+(y+Y_OFFSET)*XSize,BOMB_IMAGE._imageData);	

void vpoke(unsigned short addr, unsigned char val)
{
	POKE(VDP_Status_Write, addr & 0x00FF);
	POKE(VDP_Status_Write, (addr >> 8) | 0x40);
	POKE(VDP_Data_Write, val);
}	
...
void INIT_GRAPHICS(void)
{
		...
		vpoke(8192+ 4,10*16); // White !, $ -- 32 - 39
		vpoke(8192+ 5, 2*16); // Green  -- 40 - 47
		vpoke(8192+ 6, 4*16); // Green -- 48 - 55
		vpoke(8192+ 7, 4*16); // Green -- 56 - 63
		
		vpoke(8192+ 8, 8*16); // Red 64 --
		vpoke(8192+ 9, 8*16); // Red 72 --
		vpoke(8192+10, 8*16); // Red 80 --
		vpoke(8192+11, 8*16); // Red 88 --
	#endif
