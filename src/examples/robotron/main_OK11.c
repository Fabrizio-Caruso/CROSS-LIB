...
uint8_t enemy_dir[MAX_ENEMIES];               /* 0 left, 1 right */
...
{
    uint8_t col = _XL_CYAN;
    if (invincible_timer) col = _XL_YELLOW;
    uint8_t t0,t1;
...
{
    uint8_t i,c=0; for (i=0;i<MAX_BULLETS;i++) if (bullet_active[i]) c++; return c;
}
