...
/* Check if any alive monster is on the player's tile. Returns 1 if collision. */
...
void move_bullet(void)
{
    ...
    /* Check if a monster is already on the bullet's current position
       (e.g. a monster moved onto the bullet in a previous frame). */
    for (i = 0; i < num_monsters; i++) {
    ...
    /* Bullet hits a wall or goes out of bounds */
...
int main(void)
{
    ...
    while (1) {
        ...
        while (game_state == 0) {
            ...
            /* --- Move monsters (much slower: every 15 player moves) --- */
