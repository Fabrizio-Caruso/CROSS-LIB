...
By "universal" here I mean that these games will run on more than 200 different systems (consoles, computers, arcade boards, pocket calculators, etc.) despite using the very same game code.
For more details we refer to the GitHub page: http://www.github.com/Fabrizio-Caruso/CROSS-LIB/ and http://www.github.com/Fabrizio-Caruso/CROSS-LIB-HD/



## WHAT ARE CROSS-LIB and CROSS-LIB-HD?
...
Cross-Lib is **not** similar to Inofuto build tools. Inofuto's games have many versions but each version has its own source code.
...
`xl build myhello` (or equivalently `xl myhello`) [to build a native binary of the new project]

`xl run myhello` [to run the new project]

`xl build myhello`  c64 [to build the same project for the Commodore 64]
...
## TESTING ALL TARGETS ON THE HOST TERMINAL
...
How are we supposed to test and debug the (abstract) source code for so many targets, without the need to test it on hundreds of targets with hundreds of emulators?
We can do this by using the native terminal targets:
- `ncurses` (aka "native" ASCII target)
...
```
builds `snake` for the 20x24 screen and with 7x8 tiles (as on the Apple //)
...
where the currently possible effects are:
