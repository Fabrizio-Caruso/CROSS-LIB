        ...
        while (1)
        {
            ...
            for (j = 0; j < i; j++)
            {
                if (wx > walls[j].x + WALL_SIZE - 1)
                    dx = (signed char)wx - (signed char)walls[j].x - (WALL_SIZE - 1);
                else if (walls[j].x > wx + WALL_SIZE - 1)
                    dx = (signed char)walls[j].x - (signed char)wx - (WALL_SIZE - 1);
                else
                    dx = 0;

                if (wy > walls[j].y + WALL_SIZE - 1)
                    dy = (signed char)wy - (signed char)walls[j].y - (WALL_SIZE - 1);
                else if (walls[j].y > wy + WALL_SIZE - 1)
                    dy = (signed char)walls[j].y - (signed char)wy - (WALL_SIZE - 1);
                else
                    dy = 0;
...
void spawn_enemy(uint8_t idx, uint8_t type, uint16_t health, uint8_t min_dist)
{
    ...
    while (1)
    {
        ...
        dx = (signed char)ex - (signed char)player.x;
        dy = (signed char)ey - (signed char)player.y;
...
void draw_visibility_border(void)
{
    ...
    signed char bi;
    signed char vis;
    vis = (signed char)visibility;

    by = (YSize/2) - vis;
    for (bi = -vis; bi <= vis; bi++)
    {
        bx = XSize/2 + bi;
        if (bx >= 0 && bx < (signed char)XSize && by >= 0 && by < (signed char)YSize)
        {
            curr_tile[bx][by] = _TILE_10;
            curr_color[bx][by] = _XL_MAGENTA;
            _XL_DRAW(bx,by,_TILE_10,_XL_MAGENTA);
        }
    }
    by = (YSize/2) + vis;
    for (bi = -vis; bi <= vis; bi++)
...
void update_player(uint8_t input)
{
    ...
    {
        px = (signed char)player.x;
        py = (signed char)player.y;
...
void update_enemies(void)
{
    ...
    for (i = 0; i < NUM_ENEMIES; i++)
    {
        ...
        if (enemies[i].move_timer >= move_interval)
        {
            ...
            dx = (signed char)player.x - (signed char)enemies[i].x;
            dy = (signed char)player.y - (signed char)enemies[i].y;
        ...
        if (enemies[i].type == ENEMY_VERY_HARD || enemies[i].type == ENEMY_DRAGON)
        {
            ...
            if (enemies[i].shoot_timer >= shoot_interval)
            {
                ...
                px = (signed char)enemies[i].x;
                py = (signed char)enemies[i].y;
                switch (sdir)
                {
                    case DIR_UP: py--; break;
                    case DIR_DOWN: py++; break;
...
void update_projectiles(void)
{
    ...
    for (i = 0; i < MAX_PROJECTILES; i++)
    {
        ...
        px = (signed char)projectiles[i].x;
        py = (signed char)projectiles[i].y;
...
void render(void)
{
    ...
    signed char dx, dy;
    signed char sx, sy;
    // signed char cx, (YSize/2);
    uint8_t tile, color;
    signed char vis;
    uint8_t ptile, pcolor;
    uint8_t x, y;
    signed char wx, wy;
    signed char x_start, x_end, y_start, y_end;
    uint8_t wi;
    signed char wx2, wy2;
    ...
    for (y = 0; y < YSize; y++)
    {
        for (x = 0; x < XSize; x++)
        {
            ...
            curr_color[x][y] = EMPTY_COLOR;
        }
    }

    // (XSize/2) = (signed char)XSize / 2;
    // (YSize/2) = (signed char)YSize / 2;
    vis = (signed char)visibility;

    x_start = (XSize/2) - vis;
    if (x_start < 0) x_start = 0;
    x_end = (XSize/2) + vis;
    if (x_end >= (signed char)XSize) x_end = (signed char)XSize - 1;
    y_start = (YSize/2) - vis;
    if (y_start < 0) y_start = 0;
    y_end = (YSize/2) + vis;
    if (y_end >= (signed char)YSize) y_end = (signed char)YSize - 1;
    ...
    for (y = y_start; y < y_end; y++)
    {
        ...
        {
            wx = (signed char)player.x + (signed char)x - (XSize/2);
            wy = (signed char)player.y + (signed char)y - (YSize/2);
            if (wx < 0 || wx >= MAP_SIZE || wy < 0 || wy >= MAP_SIZE)
                continue;
            if (wx == 0 || wx == MAP_SIZE - 1 || wy == 0 || wy == MAP_SIZE - 1)
            {
                dx = wx - (signed char)player.x;
                dy = wy - (signed char)player.y;
    ...
    for (wi = 0; wi < NUM_WALLS; wi++)
    {
        for (wy2 = 0; wy2 < WALL_SIZE; wy2++)
        {
            ...
            {
                wx = (signed char)walls[wi].x + (signed char)wx2;
                wy = (signed char)walls[wi].y + (signed char)wy2;
                dx = wx - (signed char)player.x;
                dy = wy - (signed char)player.y;
                if (dx * dx + dy * dy > vis * vis) continue;
                sx = (XSize/2) + dx;
                sy = (YSize/2) + dy;
                if (sx < 0 || sx >= (signed char)XSize) continue;
                if (sy < 0 || sy >= (signed char)YSize) continue;
                curr_tile[sx][sy] = _TILE_14;
                curr_color[sx][sy] = _XL_MAGENTA;
            }
        }
    }
    ...
    for (i = 0; i < NUM_ITEMS; i++)
    {
        ...
        dx = (signed char)items[i].x - (signed char)player.x;
        dy = (signed char)items[i].y - (signed char)player.y;
        if (dx * dx + dy * dy > vis * vis) continue;
        sx = (XSize/2) + dx;
        sy = (YSize/2) + dy;
        if (sx < 0 || sx >= (signed char)XSize) continue;
    ...
    {
        if (!enemies[i].alive) continue;
        dx = (signed char)enemies[i].x - (signed char)player.x;
        dy = (signed char)enemies[i].y - (signed char)player.y;
        if (dx * dx + dy * dy > vis * vis) continue;
        sx = (XSize/2) + dx;
        sy = (YSize/2) + dy;
        if (sx < 0 || sx >= (signed char)XSize) continue;
    ...
    {
        if (!projectiles[i].alive) continue;
        dx = (signed char)projectiles[i].x - (signed char)player.x;
        dy = (signed char)projectiles[i].y - (signed char)player.y;
        if (dx * dx + dy * dy > vis * vis) continue;
        sx = (XSize/2) + dx;
        sy = (YSize/2) + dy;
        if (sx < 0 || sx >= (signed char)XSize) continue;
    ...
    if (player.invincible_timer > 0)
        ...
        pcolor = _XL_GREEN;
    curr_tile[(XSize/2)][(YSize/2)] = ptile;
    curr_color[(XSize/2)][(YSize/2)] = pcolor;
