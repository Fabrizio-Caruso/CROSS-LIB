...
{
	return level >= FIRST_BUBBLES_LEVEL && (level%5)>=3; //!missileLevel(level) && !missileLevel(level-1) && !missile(level-2);
}
...
// void prepareDeadGhost(Character *ghost, unsigned char pos)
// {
	// PING_SOUND(); 
