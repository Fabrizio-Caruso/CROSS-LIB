...
static void draw_item(uint8_t x,uint8_t y,uint8_t t)
{
    ...
    if (t==0) col = _XL_CYAN;      /* invincibility */
    else if (t==1) col = _XL_YELLOW;/* bomb */
    else col = _XL_GREEN;          /* fire power */
    ...
    for (i=0;i<MAX_BULLETS;i++){
        ...
        for (j=0;j<MAX_ENEMIES;j++){
            if (enemy_active[j] && bullet_x[i]==enemy_x[j] && (bullet_y[i]==enemy_y[j]||bullet_y[i]==enemy_y[j]+1)){
                ...
                if (_XL_RAND()%100<60){ /* more frequent drops */
                    ...
                    for (k=0;k<MAX_ITEMS;k++) if (!item_active[k]){
                        ...
                        draw_item(item_x[k],item_y[k],t); break;
                    }
                }
            }
        }
    ...
    for (i=0;i<MAX_ENEMIES;i++){
        if (enemy_active[i]!=enemy_prev_active[i]){
            ...
            if (enemy_x[i]!=enemy_prev_x[i]||enemy_y[i]!=enemy_prev_y[i]){
                del_enemy(enemy_prev_x[i],enemy_prev_y[i]); draw_enemy(enemy_x[i],enemy_y[i]);
                enemy_prev_x[i]=enemy_x[i]; enemy_prev_y[i]=enemy_y[i];
            }
        }
    }

    /* redraw items every frame so they stay visible */
    for (i=0;i<MAX_ITEMS;i++) if (item_active[i]) draw_item(item_x[i],item_y[i],item_type[i]);

    for (i=0;i<MAX_ITEMS;i++){
        if (!item_active[i]) continue;
        if (player_x==item_x[i] && player_y==item_y[i]){
