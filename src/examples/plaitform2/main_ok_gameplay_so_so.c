...
static void generate_level(uint16_t lvl) {
    ...
    for (i = 1; i < num_plats; i++) {
        ...
        /* Constrain width so new platform fits within previous one's range */
        w = (uint8_t)(sx / 3 + my_rand() % (sx / 2));
        if (w < 6) w = 6;
        if ((short)w > sx - 1) w = (uint8_t)sx - 1;

        /* Place x1 so new platform is fully within previous one's span */
