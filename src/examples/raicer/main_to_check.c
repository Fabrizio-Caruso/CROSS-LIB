 ...
 * player_hx is a doubled horizontal coordinate.
 *
 ...
 *                      right = (player_hx + 1) / 2
 */
#define PLAYER_MIN_HX ((uint16_t)(ROAD_LEFT * 2u))
#define PLAYER_MAX_HX ((uint16_t)(ROAD_RIGHT * 2u))
...
uint8_t player_right_col(void)
{
    ...
    if (player_hx & 1u) {
        /* Odd half-tile position uses two adjacent columns. */
        right = (player_hx + 1u) / 2u;

        /* Never go beyond the original road limits. */
        if (right > ROAD_RIGHT) {
            return ROAD_RIGHT;
        }
        ...
        /* Even position uses only one column. */
...
{
    _XL_DRAW(x, (uint8_t)(YSize - 2u), _TILE_6, _XL_CYAN); /* top tile   */
    _XL_DRAW(x, (uint8_t)(YSize - 1u), _TILE_5, _XL_CYAN); /* bottom tile*/
}

/* Delete one vertical car column at x and restore road underneath. */
void delete_player_column(uint8_t x)
{
    _XL_DELETE(x, (uint8_t)(YSize - 2u));
    draw_road_tile(x, (uint8_t)(YSize - 2u));

    _XL_DELETE(x, (uint8_t)(YSize - 1u));
    draw_road_tile(x, (uint8_t)(YSize - 1u));
}
...
void player_move_left(void)
{
    uint8_t old_left;
    uint8_t old_right;
    uint16_t min_player_hx;

    min_player_hx = PLAYER_MIN_HX;

    /* Do not leave the original left border. */
    if (player_hx <= min_player_hx) {
        return;
    }
    ...
    if (player_hx & 1u) {
        ...
        if (old_left > ROAD_LEFT) {
            draw_player_column((uint8_t)(old_left - 1u));
        }
    }
}

/* Move player one half-tile to the right, clamped to original road limits. */
void player_move_right(void)
{
    ...
    max_player_hx = PLAYER_MAX_HX;

    /* Do not produce tiles beyond the previous right limit. */
    if (player_hx >= max_player_hx) {
        return;
    }
    ...
    if (!(player_hx & 1u)) {
        ...
        if (old_right < ROAD_RIGHT) {
            draw_player_column((uint8_t)(old_right + 1u));
        }
    } else {
        /*
         * Currently odd: columns [old_left, old_right].
...
void delete_obstacle_tiles(uint8_t x, uint8_t y_top)
{
    ...
    if ((uint16_t)y_top + 1u < (uint16_t)YSize) {
        /* Restore bottom tile. The temporary DRAW matches original style,
           then the tile is deleted and road restored underneath it. */
        _XL_DRAW(x, (uint8_t)(y_top + 1u), _TILE_4, _XL_RED);
        _XL_DELETE(x, (uint8_t)(y_top + 1u));
        draw_road_tile(x, (uint8_t)(y_top + 1u));
    }
}
...
void update_obstacles(void)
{
    ...
    for (i = 0; i < MAX_OBSTACLES; i++) {
        ...
        /* Stop lateral drift near the bottom, same as original. */
        ...
        /* Delete old position tiles and restore road. */
        _XL_DELETE(old_x, old_y);
        draw_road_tile(old_x, old_y);

        if ((uint16_t)old_y + 1u < (uint16_t)YSize) {
            _XL_DELETE(old_x, (uint8_t)(old_y + 1u));
            draw_road_tile(old_x, (uint8_t)(old_y + 1u));
        }
...
uint8_t check_collision(void)
{
     ...
     * Obstacles collide when their top tile is at or below row YSize-3,
     * matching the original vertical overlap behavior.
     */
...
void init_game(void)
{
    ...
    start_hx = (uint16_t)(ROAD_CENTER * 2u);

    if (start_hx < PLAYER_MIN_HX) {
        start_hx = PLAYER_MIN_HX;
    } else if (start_hx > PLAYER_MAX_HX) {
        start_hx = PLAYER_MAX_HX;
    }
...
int main(void)
{
    ...
    for (;;) {
        ...
        while (!game_over) {
            ...
            if (_XL_LEFT(input)) {
                old_player_hx = player_hx;
                player_move_left();

                if (player_hx != old_player_hx) {
                    _XL_TICK_SOUND();
                }
