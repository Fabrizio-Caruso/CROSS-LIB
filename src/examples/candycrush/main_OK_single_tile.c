...
static void draw_cell(uint8_t gx, uint8_t gy)
{
    ...
    if (tile != g_disp_tile[gx][gy] || col != g_disp_col[gx][gy]) {
        sx = (uint8_t)(gx * TW);
        sy = (uint8_t)(gy * TH + BOFF);
        _XL_DRAW(sx, sy, tile, col);
        _XL_DRAW((uint8_t)(sx + 1), sy, tile, col);
        _XL_DRAW(sx, (uint8_t)(sy + 1), tile, col);
        _XL_DRAW((uint8_t)(sx + 1), (uint8_t)(sy + 1), tile, col);
        g_disp_tile[gx][gy] = tile;
...
static void show_explosion(void)
{
    ...
    for (y = 0; y < BS; y++) {
        for (x = 0; x < BS; x++) {
            if (g_mrk[x][y]) {
                sx = (uint8_t)(x * TW);
                sy = (uint8_t)(y * TH + BOFF);
                _XL_DRAW(sx, sy, _TILE_26, _XL_WHITE);
                _XL_DRAW((uint8_t)(sx + 1), sy, _TILE_26, _XL_WHITE);
                _XL_DRAW(sx, (uint8_t)(sy + 1), _TILE_26, _XL_WHITE);
                _XL_DRAW((uint8_t)(sx + 1), (uint8_t)(sy + 1), _TILE_26, _XL_WHITE);
                g_disp_tile[x][y] = _TILE_26;
...
int main(void)
{
    ...
    for (;;) {
        ...
        _XL_PRINT(0, 0, "CANDY CRUSH");
        _XL_SET_TEXT_COLOR(_XL_WHITE);
        _XL_PRINT(0, 2, "ARROWS:MOVE");
        _XL_PRINT(0, 4, "FIRE:SELECT");
        _XL_PRINT(0, 6, "ARROW:SWAP");
        _XL_SLEEP(2);
        _XL_CLEAR_SCREEN();
        ...
        _XL_SET_TEXT_COLOR(_XL_YELLOW);
        _XL_PRINT(0, 2, "SCORE");
        _XL_PRINTD(5, 2, 1, g_score);
        _XL_SET_TEXT_COLOR(_XL_CYAN);
        _XL_PRINT(0, 4, "PRESS ANY KEY");
        _XL_WAIT_FOR_INPUT();
    }
