...
#define X_BUFFER_OFFSET 5
#define Y_BUFFER_OFFSET 5
...
#define MAX_BUFFER_SIZE (1+(2*INITIAL_VISIBILITY))

static uint8_t prev_tile[MAX_BUFFER_SIZE][MAX_BUFFER_SIZE];
static uint8_t prev_color[MAX_BUFFER_SIZE][MAX_BUFFER_SIZE];
static uint8_t curr_tile[MAX_BUFFER_SIZE][MAX_BUFFER_SIZE];
static uint8_t curr_color[MAX_BUFFER_SIZE][MAX_BUFFER_SIZE];
...
void spawn_enemy(uint8_t idx, uint8_t type, uint16_t health, uint8_t min_dist)
{
    ...
    while (1)
    {
        ex = (uint8_t)(_XL_RAND() % MAP_SIZE);
        ey = (uint8_t)(_XL_RAND() % MAP_SIZE);
...
void render(void)
{
    ...
    /* Zero out current frame buffer */
    for (y = 0; y < MAX_BUFFER_SIZE; y++)
    {
    ...
    /* Record enemies (overwrites items on overlap) */
    for (i = 0; i < NUM_ENEMIES; i++)
    {
        if (!enemies[i].alive) continue;
    ...
    /* Compare current with previous, only draw/delete what changed */
    for (y = 0; y < MAX_BUFFER_SIZE; y++)
    {
        for (x = 0; x < MAX_BUFFER_SIZE; x++)
        {
            if (curr_tile[x][y] != prev_tile[x][y])// || curr_color[x][y] != prev_color[x][y])
            {
                ...
                {
                    _XL_DRAW(x+X_BUFFER_OFFSET, y+Y_BUFFER_OFFSET, curr_tile[x][y], curr_color[x][y]);
                }
                else
                {
                    _XL_DELETE(x+X_BUFFER_OFFSET, y+Y_BUFFER_OFFSET);
                }
            }
