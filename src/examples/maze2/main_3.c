...
uint8_t gem_x[NUM_GEMS];
uint8_t gem_y[NUM_GEMS];
uint8_t gem_alive[NUM_GEMS];
...
/* Returns 1 if position is occupied by any placed item */
uint8_t pos_taken(uint8_t px, uint8_t py)
{
    ...
    for (i = 0; i < NUM_GEMS; i++) {
        if (gem_alive[i] && gem_x[i] == px && gem_y[i] == py) return 1;
    }
    return 0;
}
...
void place_items(void)
{
    ...
    /* 4. Monsters (pos_taken checks player start, exit, key, torch, other monsters) */
    for (i = 0; i < num_monsters; i++) {
        do {
            rx = (uint8_t)(_XL_RAND() % ROOMS);
            ry = (uint8_t)(_XL_RAND() % ROOMS);
    ...
    /* 5. Gems (pos_taken checks all above; also check against previously placed gems) */
    for (i = 0; i < NUM_GEMS; i++) {
        do {
            rx = (uint8_t)(_XL_RAND() % ROOMS);
            ry = (uint8_t)(_XL_RAND() % ROOMS);
            ...
            if (pos_taken(rx * 2, ry * 2)) ok = 0;
            for (j = 0; j < i; j++) {
                if (gem_alive[j] && gem_x[j] == rx * 2 && gem_y[j] == ry * 2) ok = 0;
            }
        } while (!ok);
        gem_x[i] = rx * 2;
        gem_y[i] = ry * 2;
...
uint8_t check_monster_player_collision(void)
{
    uint8_t i;
    for (i = 0; i < num_monsters; i++) {
...
void move_bullet(void)
{
    ...
    /* Check if a monster is on the bullet's current position */
    for (i = 0; i < num_monsters; i++) {
        if (monster_alive[i] && monster_x[i] == bullet_x && monster_y[i] == bullet_y) {
            monster_alive[i] = 0;
            bullet_active = 0;
            score += 100;
            _XL_PING_SOUND();
    ...
    /* Check if the bullet's next position has a monster */
    for (i = 0; i < num_monsters; i++) {
...
void draw_screen(void)
{
    ...
    for (y = 0; y < MAZE_SIZE; y++) {
        ...
        for (x = 0; x < MAZE_SIZE; x++) {
            ...
            if (maze[x][y] == 0) {
                _XL_DRAW(x, dy, _TILE_0, _XL_BLUE);
            } else {
                _XL_DRAW(x, dy, _TILE_1, _XL_WHITE);
            }
        }
    }
...
int main(void)
{
    ...
    while (1) {
        ...
        while (game_state == 0) {
            ...
            /* Collision check BEFORE monsters move */
            if (check_monster_player_collision()) {
                game_state = 2;
                _XL_EXPLOSION_SOUND();
            }
            ...
            /* Collision check AFTER monsters move */
            if (game_state == 0 && check_monster_player_collision()) {
                game_state = 2;
                _XL_EXPLOSION_SOUND();
            }
