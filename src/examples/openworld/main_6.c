...
#define NUM_INVINCIBLE 3
#define NUM_TORCHES 2
#define NUM_HEALTH 5
#define NUM_ITEMS (NUM_HEALTH+NUM_TORCHES+NUM_INVINCIBLE+NUM_POTIONS+NUM_GEMS)
#define MAX_PROJECTILES 4
#define MAX_SHOT_RANGE 5
#define INVINCIBLE_DURATION 150
#define PLAYER_MAX_HP 9
#define MIN_SPAWN_DIST 15
#define FAR_SPAWN_DIST 35
#define INITIAL_VISIBILITY 4
...
void render(void)
{
    ...
    for (y = x_start; y < x_end; y++)
    {
        ...
        {
            wx = (short)player.x + (short)x - ((XSize)/2);
            wy = (short)player.y + (short)y - ((YSize)/2);
    ...
    /* Render HUD only when values change */
    // render_hud();
}
