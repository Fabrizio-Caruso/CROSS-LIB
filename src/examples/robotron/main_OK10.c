...
void update_game(void)
{
    ...
    uint8_t nx,ny; uint8_t occupied;
    uint8_t ox,oy;
        ...
        if (player_x==item_x[i] && (player_y==item_y[i] || player_y+1==item_y[i] || player_y==item_y[i]+1)){
            ...
            if (item_type[i]==0) invincible_timer=120;
                ...
                for (j=0;j<MAX_ENEMIES;j++) if (enemy_active[j]){
                    uint8_t dxx = enemy_x[j] > item_x[i] ? enemy_x[j]-item_x[i] : item_x[i]-enemy_x[j];
                    uint8_t dyy = enemy_y[j] > item_y[i] ? enemy_y[j]-item_y[i] : item_y[i]-enemy_y[j];
