...
void hgr_draw(uint8_t x, uint8_t y, uint8_t tile, uint8_t color)
{
    ...
    for(k=0;k<8;++k)
    {
        POKE(HB1[8*y+k]+2*x,hgr_enc[(uint16_t) (mock_tile[k])*2u+((256U)*(color))]|extra_color);//(uint16_t) 256u*color]);
        POKE(HB1[8*y+k]+2*x+1,hgr_enc[(uint16_t)(mock_tile[k])*2u+((256U)*(color))+1]|extra_color);//(uint16_t)256u*color+1]);
    }
}
