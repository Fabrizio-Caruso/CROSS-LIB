...
static const struct target_info sfx_targets[] =
{
    ...
    {3032, 0x9e, 0x0401, 0x8000,  "CBM PET 3032 BASIC2", "prg"},
    {4032, 0x9e, 0x0401, 0x8000,  "CBM PET 4032 BASIC4", "prg"},
...
void print_command_usage(const char *appl, enum log_level level)
{
    ...
    LOG(level,
        ("  To show command specific options combine the command with -?:\n"
         "  %s level -?\n"
         "  %s mem -?\n"
         "  %s sfx -?\n"
         "  %s raw -?\n"
...
void sfx(const char *appl, int argc, char *argv[])
{
    ...
    do
    {
        ...
        else if(strcmp(p, "basic") == 0)
        {
            ...
            if(p == NULL) break;
            if(strcmp(p, "*") != 0 &&
               str_to_int(p, &basic_txt_start, NULL) != 0)
            {
                LOG(LOG_ERROR,
                    ("Error: invalid value for the start of basic text "
                     "address.\n"));
            ...
            if(p == NULL) break;
            if(strcmp(p, "*") != 0 &&
               str_to_int(p, &basic_var_start, NULL) != 0)
            {
                LOG(LOG_ERROR,
            ...
            if(p == NULL) break;
            if(strcmp(p, "*") != 0 &&
               str_to_int(p, &basic_highest_addr, NULL) != 0)
            {
                LOG(LOG_ERROR,
...

    {
        ...
        else
        {
            ...
            if (transfer_len > 0)
            {
                LOG(LOG_NORMAL, (" Setup copy from | $%04X| $%04X| to $%04X\n",
                                 highest_addr_out - transfer_len,
                                 highest_addr_out, lowest_addr));
            }
