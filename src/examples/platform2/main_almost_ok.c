...
static void generate_level(uint16_t lvl) {
    ...
    short sx;
    uint8_t overlap_start, overlap_end;
    ...
    platforms[plat_count].x1 = 2;
    platforms[plat_count].x2 = (uint8_t)(XSize - 3);
    platforms[plat_count].y = (uint8_t)(YSize - 2);
    plat_count++;

    for (i = 1; i < num_plats; i++) {
        uint8_t w, x1, y, prev_x1, prev_x2;
        prev_x1 = platforms[i - 1].x1;
        prev_x2 = platforms[i - 1].x2;
        w = (uint8_t)(4 + my_rand() % 5);
        if (w > XSize - 4) w = (uint8_t)(XSize - 4);
        sx = (short)prev_x2 - (short)prev_x1 + 1;
        x1 = (uint8_t)(prev_x1 + my_rand() % ((uint16_t)sx));
        if ((uint8_t)(x1 + w) > XSize - 2) x1 = (uint8_t)(XSize - 2 - w);
        ...
        overlap_start = prev_x1;
        if (platforms[i].x1 > overlap_start) overlap_start = platforms[i].x1;
        overlap_end = prev_x2;
        if (platforms[i].x2 < overlap_end) overlap_end = platforms[i].x2;
        if (overlap_start <= overlap_end && elev_count < MAX_ELEVATORS) {
            sx = (short)overlap_end - (short)overlap_start + 1;
            {
                uint8_t ex;
                ex = (uint8_t)(overlap_start + my_rand() % ((uint16_t)sx));
                elevators[elev_count].x = ex;
                elevators[elev_count].yl = platforms[i].y;
                elevators[elev_count].yu = platforms[i - 1].y;
                elev_count++;
            }
        }
    }
    ...
    for (i = 0; i < DIAMOND_COUNT && item_idx < MAX_ITEMS; i++) {
        uint8_t pi, px;
        pi = (uint8_t)(my_rand() % plat_count);
        if (pi == 0) pi = (uint8_t)(1 + my_rand() % (plat_count - 1));
    ...
    if (enemy_count == 0 && num_plats > 1) {
        uint8_t pi, ex2;
        pi = (uint8_t)(1 + my_rand() % (num_plats - 1));
        sx = (short)platforms[pi].x2 - (short)platforms[pi].x1;
        ex2 = (uint8_t)(platforms[pi].x1 + my_rand() % ((uint16_t)sx + 1));
        enemies[enemy_count].x = ex2;
        enemies[enemy_count].y = platforms[pi].y;
        if (my_rand() & 1) enemies[enemy_count].dir = 0; else enemies[enemy_count].dir = 1;
        enemies[enemy_count].alive = 1;
        enemy_count++;
    }
    ...
    for (i = 0; i < MAX_BULLETS; i++) {
        ...
        for (j = 0; j < enemy_count; j++) {
            ...
            if ((bullets[i].x == enemies[j].x) &&
                ((bullets[i].y == (uint8_t)(enemies[j].y - 2)) ||
                 (bullets[i].y == (uint8_t)(enemies[j].y - 1)))) {
                safe_delete(enemies[j].x, (uint8_t)(enemies[j].y - 2));
                safe_delete(enemies[j].x, (uint8_t)(enemies[j].y - 1));
                enemies[j].alive = 0;
                bullets[i].active = 0;
                score += 50;
                _XL_PING_SOUND();
...
static void play_level(void) {
    ...
    while (!game_over && !level_complete) {
        ...
        if (!on_platform(player_x, player_y)) {
            uint8_t found = 0;
            for (i = 0; i < plat_count && !found; i++) {
                if (player_x >= platforms[i].x1 && player_x <= platforms[i].x2) {
                    if (platforms[i].y > player_y) {
                        erase_player();
                        while (player_y < platforms[i].y) {
                            player_y++;
                        }
                        draw_player();
                        found = 1;
                    } else if (platforms[i].y == (uint8_t)(player_y - 2) || platforms[i].y == (uint8_t)(player_y - 1)) {
                        found = 1;
                    }
                }
            }
        ...
        if (_XL_FIRE(input) && gun_ammo > 0) {
            ...
            for (i = 0; i < MAX_BULLETS && !fired; i++) {
                if (!bullets[i].active) {
                    ...
                    bullets[i].y = (uint8_t)(player_y - 1);
