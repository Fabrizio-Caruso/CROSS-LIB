...
static void place_enemies(uint8_t lvl)
{
    ...
    min_dist = (uint8_t)((XSize + YSize) / 3);
...
static void gen_maze_level(uint8_t lvl)
{
    ...
    cx = px; cy = py;
    steps = 0;
    max_steps = (uint16_t)(XSize * YSize);
    ...
    for (i = 0; i < XSize / 4 + YSize / 5; i++) {
        sx = (uint8_t)(1 + _XL_RAND() % (XSize - 4));
        sy = (uint8_t)(1 + _XL_RAND() % (YSize - 4));
...
static void gen_open_room(uint8_t lvl)
{
    ...
    for (i = 0; i < num_blocks; i++) {
        ...
        bx = (uint8_t)((XSize / 4) + _XL_RAND() % (XSize / 2));
        by = (uint8_t)((YSize / 4) + _XL_RAND() % (YSize / 2));
...
static void render(void)
{
    ...
    for (i = 0; i < enemy_count; i++) {
        ecol = (shield_timer > 0) ? _XL_CYAN : _XL_RED;
        buf_draw(ex[i], ey[i], T_ENEMY, ecol);
    }
    ...
    pcol = (shield_timer > 0) ? _XL_YELLOW : _XL_CYAN;
...
static void move_enemies(void)
{
    ...
    for (i = 0; i < enemy_count; i++) {
        dx = (int16_t)px - (int16_t)ex[i];
        dy = (int16_t)py - (int16_t)ey[i];
