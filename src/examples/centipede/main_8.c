...
static void move_piece(uint8_t pi)
{
    ...
    if (piece_in_area[pi]) {
        ...
        /* bounce off the bottom border: move up and reverse direction */
...
static void update_bullets(void)
{
    ...
    for (i = 0; i < MAX_BULLETS; i++) {
        ...
        /* --- check collision at current position (old_y) --- */
        ...
        /* --- check collision at intermediate position (mid_y) --- */
        for (j = 0; j < MAX_SEGS; j++) {
        ...
        /* --- check collision at final position (new_y) --- */
        for (j = 0; j < MAX_SEGS; j++) {
        ...
        /* no collision: move bullet to new_y */
        _XL_DELETE(bul_x[i], old_y);
        bul_y[i] = new_y;
...
int main(void)
{
    ...
    while (1) {
        ...
        if (pl_alive) {
            if (_XL_LEFT(input)) {
                nx = (pl_x > 0) ? (uint8_t)(pl_x - 1) : 0;
                can_move = 1;
                if (player_blocked(nx, pl_y)) can_move = 0;
                if (can_move) {
            ...
            if (_XL_RIGHT(input)) {
                nx = (pl_x < XSize - 1) ? (uint8_t)(pl_x + 1) : (uint8_t)(XSize - 1);
                can_move = 1;
                if (player_blocked(nx, pl_y)) can_move = 0;
                if (can_move) {
                    check_player_collision();
            ...
            if (_XL_UP(input)) {
                ny = (pl_y > PLAYER_MIN_Y) ? (uint8_t)(pl_y - 1) : PLAYER_MIN_Y;
                can_move = 1;
                if (player_blocked(pl_x, ny)) can_move = 0;
                if (can_move) {
                    check_player_collision();
                    if (pl_alive) {
                        erase_player();
            ...
            if (_XL_DOWN(input)) {
                ny = (pl_y < PLAYER_MAX_Y) ? (uint8_t)(pl_y + 1) : PLAYER_MAX_Y;
                can_move = 1;
                if (player_blocked(pl_x, ny)) can_move = 0;
                if (can_move) {
                    check_player_collision();
