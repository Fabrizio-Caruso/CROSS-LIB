...
typedef struct {
    uint8_t x, y, dir, type, alive, range;
} Projectile;
...
void init_game(void)
{
    ...
    for (i = 0; i < NUM_ENEMIES; i++)
    {
        ...
        if (i < NUM_EASY)
        {
            enemies[i].type = ENEMY_EASY;
            enemies[i].health = 1;
            rx = (uint16_t)(_XL_RAND() % 21);
            ry = (uint16_t)(_XL_RAND() % 21);
            sx = (short)rx - 10;
            sy = (short)ry - 10;
        }
        else if (i < NUM_EASY + NUM_HARD)
        {
            enemies[i].type = ENEMY_HARD;
            enemies[i].health = 6;
            rx = (uint16_t)(_XL_RAND() % 41);
            ry = (uint16_t)(_XL_RAND() % 41);
            sx = (short)rx - 20;
            sy = (short)ry - 20;
        }
        else if (i < NUM_EASY + NUM_HARD + NUM_VERY_HARD)
        {
            enemies[i].type = ENEMY_VERY_HARD;
            enemies[i].health = 20;
            rx = (uint16_t)(_XL_RAND() % 16);
            ry = (uint16_t)(_XL_RAND() % 16);
            if (_XL_RAND() % 2) { sx = (short)rx; } else { sx = (short)rx - 45; }
            if (_XL_RAND() % 2) { sy = (short)ry; } else { sy = (short)ry - 45; }
        }
        else
        {
            enemies[i].type = ENEMY_DRAGON;
            enemies[i].health = 200;
            rx = (uint16_t)(_XL_RAND() % 11);
            ry = (uint16_t)(_XL_RAND() % 11);
            if (_XL_RAND() % 2) { sx = (short)rx; } else { sx = (short)rx - 49; }
            if (_XL_RAND() % 2) { sy = (short)ry; } else { sy = (short)ry - 49; }
        }

        enemies[i].x = (uint8_t)(player.x + sx);
        enemies[i].y = (uint8_t)(player.y + sy);
        if (enemies[i].x >= MAP_SIZE) enemies[i].x = MAP_SIZE - 1;
        if (enemies[i].y >= MAP_SIZE) enemies[i].y = MAP_SIZE - 1;
    }
    ...
    for (i = 0; i < NUM_ITEMS; i++)
    {
        ...
        items[i].x = (uint8_t)(_XL_RAND() % MAP_SIZE);
        items[i].y = (uint8_t)(_XL_RAND() % MAP_SIZE);
    }
...
void update_enemies(void)
{
    ...
    for (i = 0; i < NUM_ENEMIES; i++)
    {
        ...
        enemies[i].move_timer++;
        {
            ...
            if (enemies[i].type == ENEMY_EASY) move_interval = 3;
            else if (enemies[i].type == ENEMY_HARD) move_interval = 4;
            else if (enemies[i].type == ENEMY_VERY_HARD) move_interval = 5;
            else move_interval = 6;
            ...
            if (enemies[i].move_timer >= move_interval)
            {
                ...
                if (dx > 0)
                {
                    if (enemies[i].x < MAP_SIZE - 1) enemies[i].x++;
                }
                else if (dx < 0)
                {
                ...
                if (dy > 0)
                {
                    if (enemies[i].y < MAP_SIZE - 1) enemies[i].y++;
                }
                else if (dy < 0)
                {
        ...
        if (enemies[i].type == ENEMY_VERY_HARD || enemies[i].type == ENEMY_DRAGON)
        {
            ...
            if (enemies[i].type == ENEMY_DRAGON) shoot_interval = 20;
            else shoot_interval = 30;
...
void render(void)
{
    ...
    for (i = 0; i < NUM_ITEMS; i++)
    {
        ...
        if (dx * dx + dy * dy > VISIBILITY * VISIBILITY) continue;
        sx = cx + dx;
        sy = cy + dy;
        ...
        _XL_DRAW((uint8_t)sx, (uint8_t)sy, tile, color);
    }
...

    {
        ...
        if (player.invincible_timer > 0)
            ...
            pcolor = _XL_GREEN;
        _XL_DRAW((uint8_t)cx, (uint8_t)cy, ptile, pcolor);
    }
