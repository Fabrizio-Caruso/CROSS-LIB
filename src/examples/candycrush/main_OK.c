...
static uint8_t candy_tiles[NC][4] = {
    ...
    {_TILE_23, _TILE_24, _TILE_25, _TILE_26}
};
        ...
        for (x = 0; x < BS; x++) {
            if (g_mrk[x][y]) {
                ...
                for (q = 0; q < 4; q++) {
                    dx = (uint8_t)(sx + (q % 2));
                    dy = (uint8_t)(sy + (q / 2));
                    _XL_DRAW(dx, dy, _TILE_27, _XL_WHITE);
                    g_disp_tile[x][y][q] = _TILE_27;
...
int main(void)
{
    ...
    for (;;) {
        ...
        _XL_SET_TEXT_COLOR(_XL_WHITE);
        _XL_PRINT(0, 2, "ARROWS TO MOVE");
        _XL_PRINT(1, 2, "FIRE TO SELECT");
        _XL_PRINT(2, 6, "ARROW TO SWAP");
        _XL_SLEEP(2);
        _XL_CLEAR_SCREEN();
