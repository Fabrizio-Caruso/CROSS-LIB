...
// --- Pick a random exit corner (top-right, bottom-left, or bottom-right) ---
void pick_exit(void) {
...
// --- Maze Generation (guaranteed solvable path from (0,0) to exit) ---
void generate_maze(void) {
    ...
    // Build a guaranteed path from (0,0) to (exit_x, exit_y)
    cx = 0;
    cy = 0;
    path_len = 1;
    path_x[0] = 0;
    ...
    while (cx != exit_x || cy != exit_y) {
        if (cx != exit_x && cy != exit_y) {
            // Can move in both axes; pick randomly
            if ((_XL_RAND() % 2) == 0) {
                cx += (exit_x > cx) ? 1 : -1;
            } else {
                cy += (exit_y > cy) ? 1 : -1;
...
// --- BFS flood fill from (0,0) to mark all reachable cells ---
...
// --- Place Items (only on reachable floor cells) ---
...
// --- Place Monsters (only on reachable floor cells) ---
...
// --- End Screen (waits for key, then returns) ---
...
int main(void) {
    ...
    while (1) {
        ...
        _XL_PRINT(XSize / 4, YSize / 4 + 8, "GRAB GEMS AND TORCH");
        _XL_PRINT(XSize / 4, YSize / 4 + 10, "PRESS ANY KEY");
        _XL_WAIT_FOR_INPUT();
        ...
        // Loop back to while(1) -> reset_game() -> title -> play again
    }
