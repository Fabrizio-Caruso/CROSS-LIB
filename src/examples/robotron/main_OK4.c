...
#define ENEMY_MOVE_CHANCE 20   /* 1 / ENEMY_MOVE_CHANCE enemies move each frame */
...
void init_game(void)
{
    ...
    for (i = 0; i < 12; i++) {
        enemy_active[i] = 1;
        side = _XL_RAND() % 4;
        if (side == 0) { enemy_x[i] = 0;               enemy_y[i] = (uint8_t)(_XL_RAND() % (YSize - 2)); }
        else if (side == 1){ enemy_x[i] = XSize - 1;   enemy_y[i] = (uint8_t)(_XL_RAND() % (YSize - 2)); }
        else if (side == 2){ enemy_x[i] = (uint8_t)(_XL_RAND() % XSize); enemy_y[i] = 0; }
...
void update_game(void)
{
    ...
    if (_XL_UP(inp))   { if (player_y > 0) player_y--; last_move_dir = 0; }
