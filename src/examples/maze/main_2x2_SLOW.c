...
#define MONSTER_MOVE_INTERVAL 8
#define PLAYER_MOVE_INTERVAL 4
...
// --- Move Player (1 tile max per call) ---
void move_player(uint8_t input) {
    ...
    if (moved) {
        _XL_TICK_SOUND();
        player_move_timer = 0; // reset cooldown after a successful move
    }
}
...
int main(void) {
    ...
    while (1) {
        ...
        while (!game_over) {
            ...
            // --- Player movement (rate-limited to 1 tile per interval) ---
            player_move_timer++;
            ...
            // --- Shooting (not rate-limited, but only one bullet at a time) ---
