...
// --- Respawn player at start (same level, same maze) ---
void respawn_player(void) {
    ...
    // Re-place monsters so they aren't stacked on the player
    place_monsters();
}
    ...
    while (1) {
        ...
        while (!game_over) {
            ...
            if (player_dead) {
                lives--;
                if (lives > 0) {
                    // Respawn at start of same level
                    respawn_player();
                    _XL_SLEEP(1);
                } else {
