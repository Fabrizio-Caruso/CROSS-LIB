...
void draw_full_board(void)
{
    ...
    for (i = 0; i < BOARD_SIZE; i++) {
        for (j = 0; j < BOARD_SIZE; j++) {
            _XL_DRAW(bx + j, by + i, _TILE_0, _XL_GREEN);
            if (board[i][j] == PLAYER) {
                _XL_DRAW(bx + j, by + i, _TILE_1, _XL_YELLOW);
            } else if (board[i][j] == AI) {
                _XL_DRAW(bx + j, by + i, _TILE_2, _XL_MAGENTA);
            }
        }
    }
    if (board[cursor_y][cursor_x] == EMPTY) {
        _XL_DRAW(bx + cursor_x, by + cursor_y, _TILE_3, _XL_RED);
    }
}
...
{
    _XL_SET_TEXT_COLOR(_XL_BLACK);
    _XL_PRINT(0, by + BOARD_SIZE + 1, "                    ");
    if (winner == PLAYER) {
        _XL_SET_TEXT_COLOR(_XL_YELLOW);
        _XL_PRINT(0, by + BOARD_SIZE + 1, " YOU WIN!          ");
        player_wins++;
        _XL_EXPLOSION_SOUND();
    } else if (winner == AI) {
        _XL_SET_TEXT_COLOR(_XL_MAGENTA);
        _XL_PRINT(0, by + BOARD_SIZE + 1, " AI WINS!           ");
        ai_wins++;
        _XL_ZAP_SOUND();
    } else {
        _XL_SET_TEXT_COLOR(_XL_WHITE);
        _XL_PRINT(0, by + BOARD_SIZE + 1, " DRAW!              ");
        draw_count++;
        _XL_TOCK_SOUND();
    }
    _XL_SET_TEXT_COLOR(_XL_CYAN);
    _XL_PRINT(0, by + BOARD_SIZE + 2, " PRESS FIRE TO CONTINUE");
}
...
int main(void)
{
    ...
    while (1) {
        ...
        draw_full_board();
        _XL_SET_TEXT_COLOR(_XL_WHITE);
        _XL_PRINT(0, by + BOARD_SIZE + 1, " YOUR TURN            ");
        _XL_PING_SOUND();
        ...
        while (!game_over) {
            ...
            while (current_turn == PLAYER && !game_over) {
                ...
                if (_XL_LEFT(input)) {
                    if (cursor_x > 0) {
                        ...
                        if (board[old_cy][old_cx] == EMPTY)
                            _XL_DELETE(bx + old_cx, by + old_cy);
                        if (board[cursor_y][cursor_x] == EMPTY)
                            _XL_DRAW(bx + cursor_x, by + cursor_y, _TILE_3, _XL_RED);
                        _XL_TICK_SOUND();
                        _XL_SLOW_DOWN(_XL_SLOW_DOWN_FACTOR);
                ...
                if (_XL_FIRE(input)) {
                    if (board[cursor_y][cursor_x] == EMPTY) {
                        _XL_DELETE(bx + cursor_x, by + cursor_y);
                        board[cursor_y][cursor_x] = PLAYER;
                        _XL_DRAW(bx + cursor_x, by + cursor_y, _TILE_1, _XL_YELLOW);
                        _XL_SHOOT_SOUND();
                        ...
                        current_turn = AI;
                        _XL_SET_TEXT_COLOR(_XL_MAGENTA);
                        _XL_PRINT(0, by + BOARD_SIZE + 1, " AI THINKING...     ");
                        break;
                    }
                }
                _XL_SLOW_DOWN(_XL_SLOW_DOWN_FACTOR);
            ...
            if (!game_over && current_turn == AI) {
                ...
                ai_make_move();
                if (!game_over) {
                    _XL_DRAW(bx + ai_move_x, by + ai_move_y, _TILE_2, _XL_MAGENTA);
                    _XL_TOCK_SOUND();
