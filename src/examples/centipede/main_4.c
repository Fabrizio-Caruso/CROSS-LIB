...
/* ===== spider ===== */
/* sp_state: 0 = horizontal, 1 = jumping up, 2 = jumping down */
...
static void update_spider(void)
{
    ...
    if (sp_state == 0) {
        ...
        if (sp_zig >= 5) {
            sp_zig = 0;
            /* start jumping up to near the top of the player area */
            sp_target_y = AREA_TOP;
            if (sp_y <= sp_target_y) {
                /* already at top, jump down then back up */
                sp_target_y = (uint8_t)(AREA_TOP + 3);
                sp_state = 2;
            } else {
                sp_state = 1;
            }
        ...
        /* jumping down: one row per tick */
        if (sp_y < YSize - 1) sp_y++;
        if (sp_y >= (uint8_t)(AREA_TOP + 3)) {
            sp_state = 0;
        }
    }

    /* eat mushroom at new position */
