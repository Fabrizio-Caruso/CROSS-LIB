...
static void draw_player(uint8_t x, uint8_t y, uint8_t dir)
{
    ...
    _XL_DRAW(x,y,t0,_XL_CYAN); _XL_DRAW(x,y+1,t1,_XL_CYAN);
}
static void del_player(uint8_t x,uint8_t y){_XL_DELETE(x,y);_XL_DELETE(x,y+1);}
static void draw_enemy(uint8_t x,uint8_t y){_XL_DRAW(x,y,_TILE_20,_XL_RED);_XL_DRAW(x,y+1,_TILE_21,_XL_RED);}
...
/* helper: is cell occupied by another active enemy ? */
static uint8_t enemy_occupies(uint8_t x, uint8_t y)
{
    uint8_t i;
    for (i=0;i<MAX_ENEMIES;i++) if (enemy_active[i]) {
        if ((enemy_x[i]==x && enemy_y[i]==y) || (enemy_x[i]==x && enemy_y[i]+1==y))
            return 1;
    }
    return 0;
}
...
void init_game(void)
{
    ...
    invincible_timer = 0; frame_parity = 0;
    player_prev_x = player_x; player_prev_y = player_y; player_prev_dir = player_dir;
    for (i=0;i<MAX_ENEMIES;i++){enemy_active[i]=0;enemy_prev_active[i]=0;}
...
static uint8_t count_active_bullets(void){uint8_t i,c=0;for(i=0;i<MAX_BULLETS;i++)if(bullet_active[i])c++;return c;}

void update_game(void)
{
    uint8_t inp,i,j,k; short dx,dy; short bdx[3],bdy[3];
    uint8_t changed; uint8_t bx,by; uint8_t active_bullets;
    ...
    if (_XL_FIRE(inp) && frame_parity==0 && active_bullets < max_bullets_allowed){
        if (fire_mode){
        ...
        uint8_t shots = fire_mode?3:1;
        for (k=0;k<shots;k++){
            bx=player_x+bdx[k]; by=player_y+bdy[k];
            if (bx>=XSize||by>=YSize) continue;
    ...
    for (i=0;i<MAX_BULLETS;i++){
        ...
        uint8_t ox=bullet_x[i], oy=bullet_y[i];
        bullet_x[i]+=bullet_dx[i]; bullet_y[i]+=bullet_dy[i];
        _XL_DELETE(ox,oy);
        if (bullet_x[i]<XSize && bullet_y[i]<YSize){_XL_DRAW(bullet_x[i],bullet_y[i],_TILE_0,_XL_YELLOW);}
        else {bullet_active[i]=0;}
        for (j=0;j<MAX_ENEMIES;j++){
            if (enemy_active[j] && bullet_x[i]==enemy_x[j] && (bullet_y[i]==enemy_y[j]||bullet_y[i]==enemy_y[j]+1)){
                del_enemy(enemy_x[j],enemy_y[j]); enemy_active[j]=0; bullet_active[i]=0;
                _XL_DELETE(bullet_x[i],bullet_y[i]); score+=10; _XL_TOCK_SOUND();
                if (_XL_RAND()%100<60){
                    uint8_t t=_XL_RAND()%3;
                    for (k=0;k<MAX_ITEMS;k++) if (!item_active[k]){
                        item_active[k]=1; item_type[k]=t; item_x[k]=enemy_x[j]; item_y[k]=enemy_y[j];
                        if (!enemy_occupies(item_x[k],item_y[k])) draw_item(item_x[k],item_y[k],t);
                        break;
                    }
                }
            }
    ...
    /* move enemies only if target cell is free */
    for (i=0;i<MAX_ENEMIES;i++){
        if (!enemy_active[i]) continue;
        if (_XL_RAND()%ENEMY_MOVE_CHANCE!=0) continue;
        uint8_t nx = enemy_x[i], ny = enemy_y[i];
        dx = (player_x > enemy_x[i]) ? 1 : (player_x < enemy_x[i]) ? -1 : 0;
        dy = (player_y > enemy_y[i]) ? 1 : (player_y < enemy_y[i]) ? -1 : 0;
        if (dx && enemy_x[i]+dx<XSize && enemy_x[i]+dx>0 && !enemy_occupies(enemy_x[i]+dx,enemy_y[i])) nx = enemy_x[i]+dx;
        if (dy && enemy_y[i]+dy<YSize-1 && enemy_y[i]+dy>=PLAY_TOP && !enemy_occupies(enemy_x[i],enemy_y[i]+dy)) ny = enemy_y[i]+dy;
        /* avoid moving into another enemy */
        uint8_t occupied = 0;
        for (j=0;j<MAX_ENEMIES;j++) if (j!=i && enemy_active[j]){
            if ((enemy_x[j]==nx && enemy_y[j]==ny) || (enemy_x[j]==nx && enemy_y[j]+1==ny)) {occupied=1;break;}
        }
        if (!occupied){ enemy_x[i]=nx; enemy_y[i]=ny; }
    }
    ...
    for (i=0;i<MAX_ENEMIES;i++){
        if (enemy_active[i]!=enemy_prev_active[i]){
            ...
            if (enemy_x[i]!=enemy_prev_x[i]||enemy_y[i]!=enemy_prev_y[i]){
                /* delete old tiles only if no other enemy uses them */
                uint8_t free = 1;
                for (j=0;j<MAX_ENEMIES;j++) if (j!=i && enemy_active[j]){
                    if ((enemy_x[j]==enemy_prev_x[i] && enemy_y[j]==enemy_prev_y[i]) ||
                        (enemy_x[j]==enemy_prev_x[i] && enemy_y[j]+1==enemy_prev_y[i])) {free=0;break;}
                }
                if (free) del_enemy(enemy_prev_x[i],enemy_prev_y[i]);
                draw_enemy(enemy_x[i],enemy_y[i]);
    ...
    /* redraw items, but never on top of an enemy */
        ...
        if (player_x==item_x[i] && (player_y==item_y[i] || player_y+1==item_y[i] || player_y==item_y[i]+1)){
            ...
            if (item_type[i]==0) invincible_timer=120;
                ...
                for (j=0;j<MAX_ENEMIES;j++) if (enemy_active[j]){
                    if (abs(enemy_x[j]-item_x[i])<=rad && abs(enemy_y[j]-item_y[i])<=rad){
                        del_enemy(enemy_x[j],enemy_y[j]); enemy_active[j]=0;
                    }
                }
            }else{
                fire_upgrade_cnt++;
    ...
    for (i=0;i<MAX_ENEMIES;i++){
        if (enemy_active[i] && enemy_x[i]==player_x && (enemy_y[i]==player_y || enemy_y[i]+1==player_y)){
            if (!invincible_timer){ lives--; del_enemy(enemy_x[i],enemy_y[i]); enemy_active[i]=0; _XL_EXPLOSION_SOUND(); }
        }
    }

    {
        uint8_t spawn_thresh=3+(score/40); if (spawn_thresh>30) spawn_thresh=30;
        if (_XL_RAND()%100 < spawn_thresh){
            for (i=0;i<MAX_ENEMIES;i++) if (!enemy_active[i]){
                ...
                enemy_active[i]=1; draw_enemy(enemy_x[i],enemy_y[i]);
