...
uint8_t item_type[NUM_ITEMS];   // ITEM_GEM or ITEM_TORCH
uint8_t item_active[NUM_ITEMS]; // 1 = still on map
uint8_t view_dist;              // current vision range (4 or 8)
...
void place_items(void) {
    ...
    for (i = 0; i < NUM_ITEMS; i++) {
        ...
        do {
            ...
            if (ix == MAZE_SIZE - 1 && iy == MAZE_SIZE - 1) valid = 0;
...
void draw_screen(void) {
    ...
    for (i = view_min_x; i <= view_max_x; i++) {
        for (j = view_min_y; j <= view_max_y; j++) {
            ...
            // Items (monsters pass through, so they don't block)
...
void next_level(void) {
    ...
    bullet_active = 0;
    monster_timer = 0;
    view_dist = 4; // reset vision to default
