...
/* FIX 1: mutable shadow of enemy state so defeated enemies stay dead */
...
static void play_game(void)
{
    ...
    /* FIX 1: seed the mutable enemy-state array from the const table */
    ...
    while (game_active) {
        ...
        /* FIX 1: check the mutable array, not the const table */
        if (enemy_alive[room] != ENEMY_NONE) {
            ...
            if (enemy_hp <= 0) {
                /* FIX 1: mark the enemy as permanently defeated */
